	...
	var Machine = machina.Fsm.extend( {
		...
		setupAdmin: function() {
			...
			sequence( [
				...
				function() {
					return auth.createRole( "client" );
				},
				function() {
					return auth.changeActionRoles( "package.list", [ "client" ], "add" );
				},
				function() {
					return auth.changeActionRoles( "package.projects", [ "client" ], "add" );
				},
				function() {
				...
				function() {
					return auth.changeActionRoles( "package.upload", [ "agent" ], "add" );
				},
				function() {
		...
		states: {
			...
			checkingForAdmin: {
				_onEnter: function() {
					auth.getUsers()
						.then( function( list ) {
							var hasAdmin = _.any( list, function( user ) {
			...
			agentCredentials: {
				...
				"prompt.done": function( credentials ) {
					sequence( [
							...
							function() {
								return auth.changeUserRoles( credentials.user, [ "agent" ], "add" );
							},
							function() {
								return auth.createToken( credentials.user, credentials.token );
							}
													] )
						.then( function() {
			...
			clientCredentials: {
				...
				"prompt.done": function( credentials ) {
					...
					sequence( [
							...
							function() {
								return auth.changeUserRoles( credentials.user, [ "client" ], "add" );
							},
							function() {
